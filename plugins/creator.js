...
let handler  = async (m, { conn, command, args, usedPrefix, DevMode }) => {
 ...
 let ftroli = { key: { remoteJid: 'status@broadcast', participant: '0@s.whatsapp.net' }, message: { orderMessage: { itemCount: 9999999999, status: 1, surface: 1, message: `${command} ${type}`, orderTitle: wm, sellerJid: '0@s.whatsapp.net' } } }
  let vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;𝑹𝒂𝒉𝒎𝒂𝒏𝑶𝒇𝒇𝒊𝒄𝒊𝒂𝒍⸙;;;\nFN:𝑹𝒂𝒉𝒎𝒂𝒏𝑶𝒇𝒇𝒊𝒄𝒊𝒂𝒍⸙\nORG:𝑹𝒂𝒉𝒎𝒂𝒏𝑶𝒇𝒇𝒊𝒄𝒊𝒂𝒍⸙\nTITLE:\nitem1.TEL;waid=6281298108109:+62 812-9810-8109\nitem1.X-ABLabel:𝑹𝒂𝒉𝒎𝒂𝒏𝑶𝒇𝒇𝒊𝒄𝒊𝒂𝒍⸙\nX-WA-BIZ-DESCRIPTION:${htjava} ᴍᴇɴᴇʀɪᴍᴀ sᴀᴠᴇ ᴋᴏɴᴛᴀᴋ\nX-WA-BIZ-NAME:𝑹𝒂𝒉𝒎𝒂𝒏𝑶𝒇𝒇𝒊𝒄𝒊𝒂𝒍⸙\nEND:VCARD`
//------- NOMOR
...
let teksbio = `┆⫹⫺ 💌 Nama : Rahman
┆⫹⫺ ✉️ Nama RL : HAMBA ALLAH
┆⫹⫺ ♂️ Gender : Pria
┆⫹⫺ 🕋 Agama : Islam
┆⫹⫺ ⏰ Tanggal lahir : 23 - 10 - 2006
┆⫹⫺ 🎨 Umur : 16
┆⫹⫺ 🧮 Kelas : X
...
let teksbio2 = `┆⫹⫺ 💌 Nama : Adul Alhy
┆⫹⫺ ✉️ Nama RL : HAMBA ALLAH
┆⫹⫺ ♂️ Gender : Pria
┆⫹⫺ 🕋 Agama : Islam
┆⫹⫺ ⏰ Tanggal lahir : 01 - 04 - 2002
┆⫹⫺ 🎨 Umur : 20
┆⫹⫺ 🧮 Kelas : Lulus
┆⫹⫺ 🧩 Hobby : Banyak
┆⫹⫺ 💬 Sifat : ASIK,BAIK,RAMAH
...
{title: "⸙ Biodata Rahman", rowId: ".owner bio"},
{title: "⸙ Biodata AdulAlhy", rowId: ".owner bio2"},
...
await conn.sendMessage(m.chat, { contacts: { displayName: wm, contacts: [{ vcard }] }}, { quoted: ftroli })
 ...
 conn.send2ButtonImg(m.chat, fs.readFileSync('./media/own.jpg').buffer, teksnomor, `Halo ${tag}\nIᴛᴜ Oᴡɴᴇʀ Kᴜ⫺⫰⫹\n` + botdate, `Sewa Bot`, `.sewa`, 'Menu', `.menu`, ftroli, { contextInfo: { externalAdReply: { showAdAttribution: true,
    mediaUrl: 'https://instagram.com/rhmnknght',
    mediaType: 2, 
    description: sgc,
   ...
   conn.sendButton(m.chat, '┍┈༺ *ＢｉｏＤａｔａＲａｈｍａｎ* ༻', teksbio, ppown, [
                ['Sewa Bot', `${usedPrefix}sewa`],
                ['Menu', `${usedPrefix}menu`]
            ], m)
            break
          ...
          conn.sendButton(m.chat, '┍┈༺ *ＢｉｏＤａｔａＡｄｕｌ* ༻', teksbio2, ppown2, [
                ['Sewa Bot', `${usedPrefix}sewa`],
                ['Menu', `${usedPrefix}menu`]
            ], m)
            break
